Name:		texlive-celtic
Version:	39797
Release:	2
Summary:	A TikZ library for drawing celtic knots
Group:		Publishing
URL:		https://www.ctan.org/tex-archive/macros/latex/contrib/celtic
License:	lppl1.3
Source0:	http://mirrors.ctan.org/systems/texlive/tlnet/archive/celtic.r%{version}.tar.xz
Source1:	http://mirrors.ctan.org/systems/texlive/tlnet/archive/celtic.doc.r%{version}.tar.xz
Source2:	http://mirrors.ctan.org/systems/texlive/tlnet/archive/celtic.source.r%{version}.tar.xz
BuildArch:	noarch
BuildRequires:	texlive-tlpkg
Requires(pre):	texlive-tlpkg
Requires(post):	texlive-kpathsea

%description
The package provides a TikZ library for drawing celtic knots.

%prep
%setup -c -a1 -a2
%autopatch -p1

%build

%install
rm -rf tlpkg
mkdir -p %{buildroot}%{_texmfdistdir}
cp -a * %{buildroot}%{_texmfdistdir}

%files
%doc %{_texmfdistdir}/source/latex/celtic
%{_texmfdistdir}/tex/latex/celtic
%doc %{_texmfdistdir}/doc/latex/celtic

%post -p %{_sbindir}/texlive.post

%postun
[ "$1" -eq 0 ] && %{_sbindir}/texlive.post
